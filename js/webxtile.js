 ...
 *
 * The main entry points for data consumption are:
...
export class WebxtileLoader {
  ...
  async *_streamLeavesViaManifest(rootFile, manifestDepth, signal) {
    ...
    // Pre-compute BFS level-start offsets for depths 0..manifestDepth.
    // levelStarts[d] = total nodes above depth d = sum(branch^l, l=0..d-1).
    const levelStarts = new Array(manifestDepth + 1);
    let s = 0;
    for (let d = 0; d <= manifestDepth; d++) { levelStarts[d] = s; s += Math.pow(branch, d); }

    // Scan one manifest tile's bitmap with an explicit DFS stack.
    // Returns { leaves: string[], boundaries: string[] }
    //   leaves     — tile names confirmed to have no children in this bitmap
    //   boundaries — tile names at depth manifestDepth that may have children
    //                covered by a deeper manifest tile
    function scanManifest(manifest, rootStem) {
        const bitmap = manifest.tiles;
        const leaves = [], boundaries = [];
        const stack  = [[0, rootStem, 0]];   // [bfsIdx, stem, depth]
        while (stack.length > 0) {
            const [idx, stem, depth] = stack.pop();
            if (!_tileExists(bitmap, idx)) continue;
            if (depth === manifestDepth) { boundaries.push(stem); continue; }
    ...
    const visitedManifests = new Set(['m_' + rootFile]);
    ...
    // Batch-fetch all identified leaf tiles.
    ...
    for (let i = 0; i < leafArr.length; i += 16) {
        ...
        const tiles = await Promise.all(batch.map(fn => this._loadTileOrNull(fn)));
        for (const tile of tiles) { if (tile) yield tile; }
    }
  }
  ...
  async *streamLeaves({ signal } = {}) {
    ...
    // Fallback: BFS traversal following children arrays (O(depth) round-trips).
